...
class Tetris(object):
	...
	def speed_up(self) -> None:
		if not hasattr(self, '_speed_counter'):
			self._speed_counter = 0

		self._speed_counter += 1


		if self._speed_counter >= 5:
			self._speed_counter = 0
			self._config['delay'] = self._config['delay'] * 0.90
			pygame.time.set_timer(pygame.USEREVENT + 1, int(self._config['delay']))
	...
	def run(self) -> None:
		...
		while 1:
			...
			if self.gameover:
				self.center_msg(f"Game Over! Scoor: {self._row_counter}\nPress space to continue")
