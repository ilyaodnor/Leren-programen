...
def dialogue_test(title: str, inputs: list = [], prompts: list =[], prints: list=[]):
  add_test_result(vars=[title], color='yellow')
  inputs_text = ''
  for input in inputs: inputs_text += (', ' if inputs_text else '') + str(input)
  if inputs_text:
    add_test_result(vars=['Inputs: '+inputs_text], color='blue')
  data['inputs'].clear()
  feed_inputs(*inputs)
  data['prints'].clear()
  expect_prints(*prints)
  data['prompts'].clear()
  expect_prompts(*prompts)
...
def str_more(s: any, max: int) -> str:
  if isinstance(s, str):
    if len(s) > max:
      s = s[:max] + "..."
    s = '"' + s + '"'
...
def test(name: str, expect: any, value: any):
  ...
  if is_regex:
    ...
    if value == expect and type_expected == type_got:
      passed = True
  if passed:
    add_test_result(txt = '{}: ' + name, vars=['success'], color='green')
    add_test_result(txt = '  for:      {}', vars=[str_more(expect,MAX_REPORT_STRING)], color='white')
    if is_regex:
      add_test_result(txt = '  with:     {}', vars=[str_more(value,MAX_REPORT_STRING)], color='white')
    
  else:
    add_test_result(txt = '{}: ' + name, vars=['failed'], color='red')
    add_test_result(txt = '  expected: {} : {}', vars=[str_type(type_expected),str_more(expect,MAX_REPORT_STRING)])
    add_test_result(txt = '  got:      {} : {}', vars=[str_type(type_got),str_more(value,MAX_REPORT_STRING)])
  add_test_result(txt = '')
  return passed
